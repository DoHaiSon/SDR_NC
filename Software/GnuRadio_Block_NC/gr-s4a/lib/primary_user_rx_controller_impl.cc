...
#include "primary_user_rx_controller_impl.h"
#include <stdexcept>
#include <iostream>
#include <string.h>
#include <cstdio>
...
namespace gr {
  namespace s4a {
    ...
    primary_user_rx_controller::sptr
    primary_user_rx_controller::make(int channel_size, int required_data_bytes_on_each_burst,
      ...
      float snr_limit, int snr_console_print_rate)
    {
      return gnuradio::get_initial_sptr
        (new primary_user_rx_controller_impl(channel_size, required_data_bytes_on_each_burst,
          working_mode, symbol_flow, node_id, remote_node_id,
     ...
     */
    primary_user_rx_controller_impl::primary_user_rx_controller_impl(int channel_size, int required_data_bytes_on_each_burst,
      int working_mode, int symbol_flow, int node_id,
      int remote_node_id, int response_timeout_ms,
      int wrong_packet_discard_limit, int tx_packet_repetition,
      float snr_limit, int snr_console_print_rate)
      : gr::block("primary_user_rx_controller",
              gr::io_signature::make3(3, 3, required_data_bytes_on_each_burst * sizeof(char),
            channel_size * sizeof(gr_complex), sizeof(float)),
              gr::io_signature::make2(
            		...
            		(int(pow(2, ceil(log2(5 + channel_size)))))
                		* sizeof(gr_complex))),
    d_channel_size(channel_size),
    d_rbytes(required_data_bytes_on_each_burst), d_node_id(node_id),
    d_remote_node_id(remote_node_id),
    d_response_timeout_ms(response_timeout_ms), d_wrong_packet_idx(0),
    d_wrong_packet_discard_limit(wrong_packet_discard_limit),
    d_tx_packet_idx(0), d_symbol_flow(symbol_flow),
    d_out_node_id(0.0), d_out_remote_node_id(0.0),
    d_tx_packet_repetition(tx_packet_repetition),
    d_snr_limit(snr_limit),
    d_snr_console_print_rate(snr_console_print_rate), d_tx_total(10),
    d_tx_idx(0), d_count(0), d_clock(0), d_elapsed_time(0.0f),
    d_out_snr(0.0)
    {
	  printf("init::primary_user_rx_controller\n");
	  printf("init::primary_user_rx_controller::node_id: \n", node_id);

	  if (working_mode == 0)
	    {
	      printf("init::primary_user_rx_controller:: MASTER\n");
	      d_state = ST_TRANSMIT_OFDM;
	      d_working_mode = MASTER;
	    }
	  else if (working_mode == 1)
	    {
	      printf("init::primary_user_rx_controller:: SLAVE\n");
	      d_state = ST_RECEIVE;
	      d_working_mode = SLAVE;
	    }
	  else
	    {
	      printf("init::primary_user_rx_controller:: SPY\n");
	      d_state = ST_RECEIVE;
	      d_working_mode = SPY;
	    }
	  ...
	  printf("init::primary_user_rx_controller::metrics_size %d\n",
		 d_metrics_size);
     ...
     */
    primary_user_rx_controller_impl::~primary_user_rx_controller_impl()
    {
    }

    void
    primary_user_rx_controller_impl::forecast (int noutput_items, gr_vector_int &ninput_items_required)
    {
    ...
    int
    primary_user_rx_controller_impl::general_work (int noutput_items,
	  ...
	  while (ni < ninput_items && no < noutput_items)
		  ...
		  if (in_snr[nidx_snr] > d_snr_limit)
		    {
		      ...
		      if (resolved_node_id == d_remote_node_id)
		        ...
		        {
		          //printf("resolved_node_id = node_id \n");
		          d_state = ST_RECEIVE;
		        }
		      else if (resolved_node_id == 0x00)
		        {
		          //printf("resolved_node_id = 0x00 \n");
		          d_state = ST_RECEIVE;
		        }
		      else
		        {
		          ...
		          //printf("resolved_node_id = node_id ELSE \n");
		          if (d_working_mode == MASTER)
		            {
		              d_wrong_packet_idx++;

		              // Check wrong packet limit!
		              if (d_wrong_packet_idx
		                  >= d_wrong_packet_discard_limit)
		                {
		  ...
		  while (d_tx_packet_idx < d_tx_packet_repetition)
		    {
		      ...
		      out_est_metrics[nodx_est_metrics + 0] = gr_complex(
		          d_out_time, 0);

		      // Push the SNR from the previous reception
		      out_est_metrics[nodx_est_metrics + 1] = gr_complex(
		          d_out_snr, 0);
		      ...
		      out_est_metrics[nodx_est_metrics + 4] = gr_complex(1,
		                                                         0);

		      // Push the channel (h) from the previous reception
		      for (i = 0; i < d_channel_size; i++)
		        {
		          out_est_metrics[nodx_est_metrics + 5 + i] =
		              d_out_h[i];
		        }
		      d_tx_packet_idx++;
		      nodx_bytes += d_rbytes;
	      ...
	      default:
		{
		  throw std::invalid_argument(
		      "primary_user_rx_controller::invalid state");
		  break;
		}
		}
	    }
