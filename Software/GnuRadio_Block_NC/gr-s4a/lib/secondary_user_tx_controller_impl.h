 ...
 *
 ******************************************************************************/
#ifndef INCLUDED_S4A_SECONDARY_USER_TX_CONTROLLER_IMPL_H
#define INCLUDED_S4A_SECONDARY_USER_TX_CONTROLLER_IMPL_H
...
class secondary_user_tx_controller_impl: public secondary_user_tx_controller
{
private:
	enum state_t
	{
...
public:
	secondary_user_tx_controller_impl(int channel_size,
			int required_data_bytes_on_each_burst, int working_mode,
			int symbol_flow, int node_id, int remote_node_id,
			int response_timeout_ms, int wrong_packet_discard_limit,
			int tx_packet_repetition, float snr_limit,
			int snr_console_print_rate);

	~secondary_user_tx_controller_impl();
...
#endif /* INCLUDED_S4A_SECONDARY_USER_TX_CONTROLLER_IMPL_H */
