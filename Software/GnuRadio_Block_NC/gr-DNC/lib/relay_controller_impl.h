...
#ifndef INCLUDED_DNC_RELAY_CONTROLLER_IMPL_H
#define INCLUDED_DNC_RELAY_CONTROLLER_IMPL_H
...
namespace gr {
  namespace DNC {
    ...
    class relay_controller_impl : public relay_controller
    {
     private:
	enum state_rx0
	{
	...
	void set_data_out(int xored_pos,unsigned char xored_pkt_no);
     ...
     public:
      relay_controller_impl(int packet_size, int guard_interval, unsigned char end_nodeA_id, unsigned char end_nodeB_id, unsigned char relay_node_id);
      ~relay_controller_impl();
...
#endif /* INCLUDED_DNC_RELAY_CONTROLLER_IMPL_H */
