...
#include "source1_controller_impl.h"

namespace gr {
  namespace DNC {

    source1_controller::sptr
    source1_controller::make(int buffer_size, int packet_size, int guard_interval, unsigned char source1_id, unsigned char source2_id, unsigned char relay1_id, unsigned char relay2_id, unsigned char destination1_id, unsigned char destination2_id, int network_coding)
    {
      return gnuradio::get_initial_sptr
        (new source1_controller_impl(buffer_size, packet_size, guard_interval, source1_id, source2_id, relay1_id, relay2_id, destination1_id, destination2_id, network_coding));
    }

    /*
     * The private constructor
     */
    source1_controller_impl::source1_controller_impl(int buffer_size, int packet_size, int guard_interval, unsigned char source1_id, unsigned char source2_id, unsigned char relay1_id, unsigned char relay2_id, unsigned char destination1_id, unsigned char destination2_id, int network_coding)
      : gr::block("source1_controller",
     ...
     */
    source1_controller_impl::~source1_controller_impl()
    {
    }

    void
    source1_controller_impl::forecast (int noutput_items, gr_vector_int &ninput_items_required)
    {
        /* <+forecast+> e.g. ninput_items_required[0] = noutput_items */
    }

    int
    source1_controller_impl::general_work (int noutput_items,
	...
	while(ni0<nInputItems0)
	{
		...
		else
		{
			switch(d_rx_state)
			{
				case ST_RX_IDLE:
				{
					if(in0[ni0]==d_relay1_id)
					{
						d_rx_node_id = d_relay1_id;
						d_rx_state = ST_RX_CHECK_NODE_ID;
						d_check_rx_id_index++;
						d_check_rx_id_count++;
					}
					if(in0[ni0]==d_destination1_id)
					{
						d_rx_node_id = d_destination1_id;
						d_rx_state = ST_RX_CHECK_NODE_ID;
						d_check_rx_id_index++;
						d_check_rx_id_count++;
					}
