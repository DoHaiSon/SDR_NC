  ...
  namespace DNC {
    ...
    end_node_controller_impl::end_node_controller_impl(int packet_size, int guard_interval, unsigned char end_node_id, unsigned char relay_node_id)
      : gr::block("end_node_controller",
	      ...
	      d_buffer_size(255), d_next_out_packet_no(0x01), d_number_of_session_packets(0), d_session_no(0x01),
    ...
    void
    end_node_controller_impl::reset()
    {
	d_request_index = 0;
	d_tx_buffer_index= 0;
		...
		if(d_data_out==true)
		{
			...
			else
			{
				...
				if(d_number_of_packet_out==d_number_of_session_packets)
				{
					...
					if(d_number_of_session_packets==255)
					{
